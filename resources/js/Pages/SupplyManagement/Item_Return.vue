...
const searchInventory = async () => {
    ...
    console.log(itemSuggestions.value);
  if (searchItem.value != "") {
...
const filteredInventories = computed(() => {
    if (searchItem.value != "") {
        ...
        return props.inventories.filter((inventory) => {
            return props.salesItems.some((item) => {
                return item.itemID == inventory.id && item.salesID == form.salesID;
            })
        });
    }
});
...
const recordStockIn = (employee, salesID, supplier) => {
    console.log(props.inventories);
...
const submit = () => {
    if(supplyOrderItems.value == undefined || supplyOrderItems.value.length == 0){
        ...
        axios.post(route('finance.store'), {
            amount: -returnCash.value, // Negative value for 'Item Return'
            generalNotes: 'Return Cash for Item Return',
            transactionType: 'Item Return',
        }).then(response => {
            console.log("Finance row stored successfully:", response.data);
        });
    }
};
...
const calculateReturn = () => {
    ...
    supplyOrderItems.value.forEach(item => {
        returnCash.value += item.quantity * (props.salesItems.find((sale) => sale.salesID == form.salesID && sale.itemID == item.itemID)?.amountPaid / props.salesItems.find((sale) => sale.salesID == form.salesID && sale.itemID == item.itemID)?.quantitySold);
    })
...
<template>
    ...
    <BreezeAuthenticatedLayout>
        ...
        <div v-if="isVisible3" class="fixed inset-0 flex items-center justify-center z-50">
            ...
            <StockInformationModal v-if="isVisible3" :employees="props.employees" :logs="props.all_inventory_logs" 
            :transaction="form.transactionType" :sales="props.sales" @confirmSubmission="recordStockIn" 
        ...
        <div v-if="isVisible" class="fixed inset-0 flex items-center justify-center z-50">
            ...
            <StockInputCard :inventory_logs="props.inventory_logs" :inventories="props.inventories"
                ...
                :salesItems="props.salesItems.filter((sale) => sale.salesID == form.salesID)"
                :itemReturns="props.all_inventory_logs.filter((log) => log.transactionType == 'Item Return' && log.salesID == form.salesID)"
                :inventory_logs_items="props.inventory_logs_items" :orderItemIndex="orderItemIndex"
                :transaction="transaction" @sendSupplyOrderItem="receiveSupplyOrderItem"
                            ...
                            <div class="w-full flex">
                                <div class="w-3/4 flex flex-col">
                                    <div class="text-3xl font-montserrat font-bold text-savoy-blue px-1">
                                        ...
                                        Return Under Sales Order #{{ form.salesID }}
                                        </div>
                                    <div class="text-sm font-montserrat text-saffron font-bold px-1">

...
const receiveSupplyOrderItem = (supplyOrderItemInstance, itemConfirmed) => {
    console.log("Item from Component: ", supplyOrderItemInstance);
...
const closeInputCard = () => {
    ...
    console.log(isVisible.value, "card cancelled");
    console.log(supplyOrderItems.value);
...
const searchInventory = async () => {
  if (searchItem.value != "") {
    ...
    itemSuggestions.value = await itemSuggestions.value.filter((item) => item.supplierID == form.supplierID);
...
const filteredInventories = computed(() => {
    if (searchItem.value != "") {
        console.log(filteredInventoryByS.value.filter((item) => item.itemName.toLowerCase().includes(searchItem.value.toLowerCase())));
        return filteredInventoryByS.value.filter((item) => item.itemName.toLowerCase().includes(searchItem.value.toLowerCase()));
    } else {
        console.log(filteredInventoryByS.value);
        return filteredInventoryByS.value;
    }
});
...
const filteredInventoryByS = ref([]);
const recordStockIn = (employee, refNum, supplier) => {
    ...
    filteredInventoryByS.value = props.inventories.filter((item) => item.supplierID == supplier);
}
    ...
    <BreezeAuthenticatedLayout>
        ...
        <div v-if="isVisible3" class="fixed inset-0 flex items-center justify-center z-50">
            ...
            <StockInformationModal v-if="isVisible3" :employees="props.employees" :logs="props.all_inventory_logs" 
                :suppliers="props.suppliers" :transaction="form.transactionType" @confirmSubmission="recordStockIn" 
                            ...
                            <div class="w-full flex">
                                <div class="w-3/4 flex flex-col">
                                    <div class="text-3xl font-montserrat font-bold text-savoy-blue px-1">
                                        Supply Log #{{ props.inventory_logs.id }}
                                    </div>
                                    <div class="text-lg font-montserrat text-savoy-blue font-bold px-1">
                                        Reference Number: {{ form.referenceNum }}
                                    </div>
                                    <div class="text-sm font-montserrat text-dark-pastel-green font-bold px-1">
                                        {{ form.transactionType }}: {{ props.suppliers.find((supplier) => supplier.id == form.supplierID).supplierName }}
                                    </div>
                                </div>

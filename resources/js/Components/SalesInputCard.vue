...
const {
  ...
  selectedItem,
  orderItem,
  orderItemIndex, } = defineProps(['sales',
    'inventories',
    'selectedItem',
    'orderItem',
    'orderItemIndex',
...
const form = useForm({
  quantity: orderItem.quantitySold || '',
  calculatedPrice: +orderItem.quantitySold * +selectedItem.itemSRP || '',
...
const setSalesOrder = () => {
  console.log("Sales Order Being Set");
  salesOrderItem.itemID = selectedItem.itemID;
  salesOrderItem.quantitySold = form.quantity;
  salesOrderItem.itemName = selectedItem.itemName;
  salesOrderItem.itemSerialNumber = selectedItem.itemSerialNumber;
  salesOrderItem.amountPaid = form.amountPaid;
  salesOrderItem.itemSRP = selectedItem.itemSRP;
  console.log(salesOrderItem);
  if (orderItemExists.value == false) {
    emit("sendSalesOrderItem", salesOrderItem, confirmItem.value);
  } else if (orderItemExists.value == true) {
    emit("updateSalesOrderItem", salesOrderItem, confirmItem.value, orderItemIndex);
  }
}
...
watchEffect(() => {
  if (orderItem.quantitySold !== undefined && orderItem.amountPaid !== undefined &&
    orderItem.quantitySold !== null && orderItem.amountPaid !== null) {
    console.log("Order Item inside: " + JSON.stringify(orderItem, null, 2));
    ...
    <div class="w-full">
      <div class="mx-auto rounded-lg overflow-hidden bg-ghost-white ">
        <div class="bg-savoy-blue text-white p-4">
          <h2 class="text-2xl font-bold font-montserrat text-center">Sales Order</h2>
        </div>
            ...
            <div class="flex flex-row space-x-4 p-3 figtree justify-center">
              ...
              <div class="flex flex-col items-center justify-center text-center">
                ...
                <div class="relative">
                  ...
                  <span class="error-message text-red-600 text-sm absolute top-full left-0"
                    v-if="isNaN(form.amountPaid) || parseFloat(form.amountPaid) > parseFloat(form.calculatedPrice) || form.amountPaid < 0" 
